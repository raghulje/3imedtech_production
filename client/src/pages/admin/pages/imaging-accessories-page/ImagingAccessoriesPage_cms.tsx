...
interface ImagingAccessoriesPageProps {
  token: string;
  showModal: boolean;
...
export default function ImagingAccessoriesPage_cms(props: ImagingAccessoriesPageProps) {
  return (
    <ProductPageBase
      pageName="imaging-accessories"
      pageTitle="Imaging Accessories"
      heroEndpoint="/api/cms/imaging-accessories/hero"
      productsEndpoint="/api/cms/imaging-accessories/products"
      {...props}
    />
  );
}

export function ImagingAccessoriesHeroForm({ formData, handleInputChange, uploadImage, uploadingImage }: any) {
  return (
    <>
      <div>
      ...
      <div>
        ...
        <input
          ...
          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          placeholder="Essential Accessories for Medical Imaging"
        />
      </div>
      <div>
        ...
        <textarea
          ...
          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          rows={4}
          placeholder="Complete range of imaging accessories to complement your medical imaging equipment"
        />
      </div>
      <div>
      ...
      <div className="flex items-center">
        <input
          type="checkbox"
          id="imagingAccessoriesHeroIsActive"
          checked={formData.isActive !== false}
          onChange={(e) => handleInputChange('isActive', e.target.checked)}
          className="mr-2"
        />
        <label htmlFor="imagingAccessoriesHeroIsActive" className="text-sm font-medium text-gray-700">Active</label>
      </div>
    </>
  );
}

export function ImagingAccessoriesProductForm({ formData, handleInputChange, uploadImage, uploadingImage }: any) {
  return (
    <>
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">Title</label>
      ...
      <div className="flex items-center">
        <input
          type="checkbox"
          id="imagingAccessoriesProductIsActive"
          checked={formData.isActive !== false}
          onChange={(e) => handleInputChange('isActive', e.target.checked)}
          className="mr-2"
        />
        <label htmlFor="imagingAccessoriesProductIsActive" className="text-sm font-medium text-gray-700">Active</label>
      </div>
    </>
  );
}
