...
interface FlatPanelDetectorsPageProps {
  token: string;
  showModal: boolean;
...
export default function FlatPanelDetectorsPage_cms(props: FlatPanelDetectorsPageProps) {
  return (
    <ProductPageBase
      pageName="flat-panel"
      pageTitle="Flat Panel Detectors"
      heroEndpoint="/api/cms/flat-panel/hero"
      productsEndpoint="/api/cms/flat-panel/products"
      {...props}
    />
  );
}

export function FlatPanelDetectorsHeroForm({ formData, handleInputChange, uploadImage, uploadingImage }: any) {
  return (
    <>
      <div>
      ...
      <div className="flex items-center">
        <input
          type="checkbox"
          id="flatPanelDetectorsHeroIsActive"
          checked={formData.isActive !== false}
          onChange={(e) => handleInputChange('isActive', e.target.checked)}
          className="mr-2"
        />
        <label htmlFor="flatPanelDetectorsHeroIsActive" className="text-sm font-medium text-gray-700">Active</label>
      </div>
    </>
  );
}

export function FlatPanelDetectorsProductForm({ formData, handleInputChange, uploadImage, uploadingImage }: any) {
  return (
    <>
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">Title</label>
      ...
      <div className="flex items-center">
        <input
          type="checkbox"
          id="flatPanelDetectorsProductIsActive"
          checked={formData.isActive !== false}
          onChange={(e) => handleInputChange('isActive', e.target.checked)}
          className="mr-2"
        />
        <label htmlFor="flatPanelDetectorsProductIsActive" className="text-sm font-medium text-gray-700">Active</label>
      </div>
    </>
  );
}
