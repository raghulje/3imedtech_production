...
interface MammographySystemsPageProps {
  token: string;
  showModal: boolean;
...
export default function MammographySystemsPage_cms(props: MammographySystemsPageProps) {
  return (
    <ProductPageBase
      pageName="mammography-systems"
      pageTitle="Mammography Systems"
      heroEndpoint="/api/cms/mammography/hero"
      productsEndpoint="/api/cms/mammography/products"
      {...props}
    />
  );
}

export function MammographySystemsHeroForm({ formData, handleInputChange, uploadImage, uploadingImage }: any) {
  return (
    <>
      <div>
      ...
      <div className="flex items-center">
        <input
          type="checkbox"
          id="mammographySystemsHeroIsActive"
          checked={formData.isActive !== false}
          onChange={(e) => handleInputChange('isActive', e.target.checked)}
          className="mr-2"
        />
        <label htmlFor="mammographySystemsHeroIsActive" className="text-sm font-medium text-gray-700">Active</label>
      </div>
    </>
  );
}

export function MammographySystemsProductForm({ formData, handleInputChange, uploadImage, uploadingImage }: any) {
  return (
    <>
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">Title</label>
      ...
      <div className="flex items-center">
        <input
          type="checkbox"
          id="mammographySystemsProductIsActive"
          checked={formData.isActive !== false}
          onChange={(e) => handleInputChange('isActive', e.target.checked)}
          className="mr-2"
        />
        <label htmlFor="mammographySystemsProductIsActive" className="text-sm font-medium text-gray-700">Active</label>
      </div>
    </>
  );
}
