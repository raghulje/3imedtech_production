...
interface RadiographySystemsPageProps {
  token: string;
  showModal: boolean;
...
export default function RadiographySystemsPage_cms(props: RadiographySystemsPageProps) {
  return (
    <ProductPageBase
      pageName="radiography-systems"
      pageTitle="Radiography Systems"
      heroEndpoint="/api/cms/radiography/hero"
      productsEndpoint="/api/cms/radiography/products"
      {...props}
    />
  );
}

export function RadiographySystemsHeroForm({ formData, handleInputChange, uploadImage, uploadingImage }: any) {
  return (
    <>
      <div>
      ...
      <div className="flex items-center">
        <input
          type="checkbox"
          id="radiographySystemsHeroIsActive"
          checked={formData.isActive !== false}
          onChange={(e) => handleInputChange('isActive', e.target.checked)}
          className="mr-2"
        />
        <label htmlFor="radiographySystemsHeroIsActive" className="text-sm font-medium text-gray-700">Active</label>
      </div>
    </>
  );
}

export function RadiographySystemsProductForm({ formData, handleInputChange, uploadImage, uploadingImage }: any) {
  return (
    <>
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">Title</label>
      ...
      <div className="flex items-center">
        <input
          type="checkbox"
          id="radiographySystemsProductIsActive"
          checked={formData.isActive !== false}
          onChange={(e) => handleInputChange('isActive', e.target.checked)}
          className="mr-2"
        />
        <label htmlFor="radiographySystemsProductIsActive" className="text-sm font-medium text-gray-700">Active</label>
      </div>
    </>
  );
}
