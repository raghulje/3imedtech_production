const { HeroSlide, Statistic, RegulatoryApproval } = require("../models");
...
exports.getAll = asyncHandler(async (req, res) => {
  const [slides, stats, regulatory] = await Promise.all([
    HeroSlide.findAll({ order: [["order", "ASC"]] }),
    Statistic.findAll({ order: [["order", "ASC"]] }),
    RegulatoryApproval.findAll({ order: [["order", "ASC"]] }),
  ]);
  return status.responseStatus(res, 200, "OK", { slides, statistics: stats, regulatory });
});
