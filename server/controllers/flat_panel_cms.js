...
const getHero = asyncHandler(async (req, res) => {
  const hero = await db.FlatPanelHero.findOne({ where: { isDeleted: false } });
...
const upsertHero = asyncHandler(async (req, res) => {
  ...
  const changeDescription = req.body.changeDescription || 'Updated Flat Panel Hero';
  
  const existing = await db.FlatPanelHero.findOne({ where: { isDeleted: false } });
  if (existing) {
    const oldData = existing.toJSON();
    await saveVersion('flat-panel-hero', null, oldData, userId, changeDescription);
    await existing.update(payload);
    return status.responseStatus(res, 200, 'Updated', existing);
  }
  const created = await db.FlatPanelHero.create(payload);
  await saveVersion('flat-panel-hero', null, created.toJSON(), userId, 'Created Flat Panel Hero');
...
const softDeleteHero = asyncHandler(async (req, res) => {
  ...
  await saveVersion('flat-panel-hero', null, oldData, userId, 'Soft deleted Flat Panel Hero');
...
const restoreHero = asyncHandler(async (req, res) => {
  const hero = await db.FlatPanelHero.findOne({ where: { isDeleted: true }, order: [['deletedAt', 'DESC']] });
  if (!hero) return status.responseStatus(res, 404, "Not found");
  ...
  const userId = req.userData?.id || null;
  await saveVersion('flat-panel-hero', null, hero.toJSON(), userId, 'Restored Flat Panel Hero');
...
const listProducts = asyncHandler(async (req, res) => {
  ...
  const products = await db.FlatPanelProduct.findAll({ 
    where,
    order: [['order', 'ASC']] 
  });
...
const createProduct = asyncHandler(async (req, res) => {
  const userId = req.userData?.id || null;
  const product = await db.FlatPanelProduct.create(req.body);
  await saveVersion('flat-panel-product', product.id, product.toJSON(), userId, 'Created Flat Panel Product');
  return status.responseStatus(res, 201, 'Created', product);
});

const updateProduct = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const product = await db.FlatPanelProduct.findByPk(id);
  ...
  const changeDescription = req.body.changeDescription || 'Updated Flat Panel Product';
  await saveVersion('flat-panel-product', product.id, oldData, userId, changeDescription);
...
const deleteProduct = asyncHandler(async (req, res) => {
  ...
  const userId = req.userData?.id || null;
  const oldData = product.toJSON();
  await saveVersion('flat-panel-product', product.id, oldData, userId, 'Soft deleted Flat Panel Product');
...
const restoreProduct = asyncHandler(async (req, res) => {
  ...
  const userId = req.userData?.id || null;
  await saveVersion('flat-panel-product', product.id, product.toJSON(), userId, 'Restored Flat Panel Product');
...
const permanentDeleteProduct = asyncHandler(async (req, res) => {
  ...
  const userId = req.userData?.id || null;
  await saveVersion('flat-panel-product', product.id, product.toJSON(), userId, 'Permanently deleted Flat Panel Product');
