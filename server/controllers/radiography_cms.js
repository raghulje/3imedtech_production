...
const getHero = asyncHandler(async (req, res) => {
  const hero = await db.RadiographyHero.findOne({ where: { isDeleted: false } });
...
const upsertHero = asyncHandler(async (req, res) => {
  ...
  const changeDescription = req.body.changeDescription || 'Updated Radiography Hero';
  
  const existing = await db.RadiographyHero.findOne({ where: { isDeleted: false } });
  if (existing) {
    const oldData = existing.toJSON();
    await saveVersion('radiography-hero', null, oldData, userId, changeDescription);
    await existing.update(payload);
    return status.responseStatus(res, 200, 'Updated', existing);
  }
  const created = await db.RadiographyHero.create(payload);
  await saveVersion('radiography-hero', null, created.toJSON(), userId, 'Created Radiography Hero');
...
const softDeleteHero = asyncHandler(async (req, res) => {
  ...
  await saveVersion('radiography-hero', null, oldData, userId, 'Soft deleted Radiography Hero');
...
const restoreHero = asyncHandler(async (req, res) => {
  const hero = await db.RadiographyHero.findOne({ where: { isDeleted: true }, order: [['deletedAt', 'DESC']] });
  if (!hero) return status.responseStatus(res, 404, "Not found");
  ...
  const userId = req.userData?.id || null;
  await saveVersion('radiography-hero', null, hero.toJSON(), userId, 'Restored Radiography Hero');
...
const listProducts = asyncHandler(async (req, res) => {
  ...
  const products = await db.RadiographyProduct.findAll({ 
    where,
    order: [['order', 'ASC']] 
  });
...
const createProduct = asyncHandler(async (req, res) => {
  const userId = req.userData?.id || null;
  const product = await db.RadiographyProduct.create(req.body);
  await saveVersion('radiography-product', product.id, product.toJSON(), userId, 'Created Radiography Product');
  return status.responseStatus(res, 201, 'Created', product);
});

const updateProduct = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const product = await db.RadiographyProduct.findByPk(id);
  ...
  const changeDescription = req.body.changeDescription || 'Updated Radiography Product';
  await saveVersion('radiography-product', product.id, oldData, userId, changeDescription);
...
const deleteProduct = asyncHandler(async (req, res) => {
  ...
  const userId = req.userData?.id || null;
  const oldData = product.toJSON();
  await saveVersion('radiography-product', product.id, oldData, userId, 'Soft deleted Radiography Product');
...
const restoreProduct = asyncHandler(async (req, res) => {
  ...
  const userId = req.userData?.id || null;
  await saveVersion('radiography-product', product.id, product.toJSON(), userId, 'Restored Radiography Product');
...
const permanentDeleteProduct = asyncHandler(async (req, res) => {
  ...
  const userId = req.userData?.id || null;
  await saveVersion('radiography-product', product.id, product.toJSON(), userId, 'Permanently deleted Radiography Product');
