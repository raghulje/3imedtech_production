...
const getHero = asyncHandler(async (req, res) => {
  const hero = await db.MammographyHero.findOne({ where: { isDeleted: false } });
...
const upsertHero = asyncHandler(async (req, res) => {
  ...
  const changeDescription = req.body.changeDescription || 'Updated Mammography Hero';
  
  const existing = await db.MammographyHero.findOne({ where: { isDeleted: false } });
  if (existing) {
    const oldData = existing.toJSON();
    await saveVersion('mammography-hero', null, oldData, userId, changeDescription);
    await existing.update(payload);
    return status.responseStatus(res, 200, 'Updated', existing);
  }
  const created = await db.MammographyHero.create(payload);
  await saveVersion('mammography-hero', null, created.toJSON(), userId, 'Created Mammography Hero');
...
const softDeleteHero = asyncHandler(async (req, res) => {
  ...
  await saveVersion('mammography-hero', null, oldData, userId, 'Soft deleted Mammography Hero');
...
const restoreHero = asyncHandler(async (req, res) => {
  const hero = await db.MammographyHero.findOne({ where: { isDeleted: true }, order: [['deletedAt', 'DESC']] });
  if (!hero) return status.responseStatus(res, 404, "Not found");
  ...
  const userId = req.userData?.id || null;
  await saveVersion('mammography-hero', null, hero.toJSON(), userId, 'Restored Mammography Hero');
...
const listProducts = asyncHandler(async (req, res) => {
  ...
  const products = await db.MammographyProduct.findAll({ 
    where,
    order: [['order', 'ASC']] 
  });
...
const createProduct = asyncHandler(async (req, res) => {
  const userId = req.userData?.id || null;
  const product = await db.MammographyProduct.create(req.body);
  await saveVersion('mammography-product', product.id, product.toJSON(), userId, 'Created Mammography Product');
  return status.responseStatus(res, 201, 'Created', product);
});

const updateProduct = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const product = await db.MammographyProduct.findByPk(id);
  ...
  const changeDescription = req.body.changeDescription || 'Updated Mammography Product';
  await saveVersion('mammography-product', product.id, oldData, userId, changeDescription);
...
const deleteProduct = asyncHandler(async (req, res) => {
  ...
  const userId = req.userData?.id || null;
  const oldData = product.toJSON();
  await saveVersion('mammography-product', product.id, oldData, userId, 'Soft deleted Mammography Product');
...
const restoreProduct = asyncHandler(async (req, res) => {
  ...
  const userId = req.userData?.id || null;
  await saveVersion('mammography-product', product.id, product.toJSON(), userId, 'Restored Mammography Product');
...
const permanentDeleteProduct = asyncHandler(async (req, res) => {
  ...
  const userId = req.userData?.id || null;
  await saveVersion('mammography-product', product.id, product.toJSON(), userId, 'Permanently deleted Mammography Product');
